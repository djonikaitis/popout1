...
% Which task it is?
ind_task = expsetup.stim.esetup_block_cond{tid};
...
while loop_over==0
    ...
    if ~isnan(t0) && (time_current - t0 >= t1) && (time_current - t0 < t1+t2)
        ...
        r1 = memory_rect;
        t1 = expsetup.stim.esetup_memory_angle(tid);
        % Plot
        if strcmp(sh1,'circle')
            Screen('FillArc', window, c1, r1, 0, 360);
        elseif strcmp(sh1,'square')
        ...
        elseif strcmp(sh1,'empty_square')
            Screen('FrameRect', window, c1, r1, p1);
        elseif strcmp(sh1, 'rectangle')
            temp1 = ones(floor(r1(3,1)-r1(1,1)), floor(r1(4,1)-r1(2,1)), 3); % Rectangle size
            for i = 1:size(temp1,3)
                temp1(:,:,i) = temp1(:,:,i)*c1(i); % Color of rectangle
            end
            tex = Screen('MakeTexture',window, temp1);
            Screen('DrawTexture', window, tex ,[], r1 , t1);
            Screen('Close', tex);
        end
    ...
    if expsetup.stim.esetup_popout_on(tid)==1
        ...
        if ~isnan(t0) && (time_current - t0 >= t1)
            ...
            r1 = popout_rect;
            c1 = expsetup.stim.esetup_popout_color(tid,:);
            p1 = expsetup.stim.esetup_target_pen_width(tid);
            t1 = expsetup.stim.esetup_popout_angle(tid);
            % Plot
            if strcmp(sh1,'circle')
                for i = 1:size(r1,2)
                    Screen('FillArc', window, c1, r1(:,i), 0, 360);
                end
            elseif strcmp(sh1,'square')
                Screen('FillRect', window, c1, r1, p1);
            elseif strcmp(sh1,'empty_circle')
                for i = 1:size(r1,2)
                    Screen('FrameArc',window, c1, r1(:,i), 0, 360, p1)
                end
            elseif strcmp(sh1,'empty_square')
                Screen('FrameRect', window, c1, r1, p1);
            elseif strcmp(sh1, 'rectangle')
                ...
                tex = Screen('MakeTexture',window, temp1);
                Screen('DrawTextures', window, tex ,[], r1 , t1);
                Screen('Close', tex);
            end
        end
    end
    ...
    if ~isnan(t0) && (time_current - t0 >= t1)
        ...
        if strcmp(expsetup.stim.esetup_target_number{tid}, 'st1') || strcmp(expsetup.stim.esetup_target_number{tid}, 'st1 & st2')
            ...
            % Properties
            sh1 = expsetup.stim.esetup_st1_shape{tid};
            r1 = st1_rect;
            c1 = expsetup.stim.esetup_st1_color(tid,:);
            p1 = expsetup.stim.esetup_target_pen_width(tid);
            t1 = expsetup.stim.esetup_st1_angle(tid);
            % Plot
            if strcmp(sh1,'circle')
                Screen('FillArc', window, c1, r1, 0, 360);
            elseif strcmp(sh1,'square')
        ...
        if isnan(expsetup.stim.edata_st1_on(tid,1))
    ...
    if ~isnan(t0) && (time_current - t0 >= t1) && nansum(expsetup.stim.eframes_st2_off{tid}(:,1))==0
        ...
        if strcmp(expsetup.stim.esetup_target_number{tid}, 'st2') || strcmp(expsetup.stim.esetup_target_number{tid}, 'st1 & st2')
            ...
            % Properties
            sh1 = expsetup.stim.esetup_st2_shape{tid};
            r1 = st2_rect;
            c1 = expsetup.stim.esetup_st2_color(tid,:);
            p1 = expsetup.stim.esetup_target_pen_width(tid);
            t1 = expsetup.stim.esetup_st2_angle(tid);
            % Plot
            if strcmp(sh1,'circle')
                Screen('FillArc', window, c1, r1, 0, 360);
            elseif strcmp(sh1,'square')
    ...
    if strcmp(expsetup.stim.esetup_target_number{tid}, 'st1 & st2') || strcmp(expsetup.stim.esetup_target_number{tid}, 'st2')
...
if strcmp(expsetup.stim.esetup_exp_version{tid}, 'final version') || ...
        strcmp(expsetup.stim.esetup_exp_version{tid}, 'target soa decrease') || strcmp(expsetup.stim.esetup_exp_version{tid}, 'target soa 0') ||...
        strcmp(expsetup.stim.esetup_exp_version{tid}, 'memory delay increase') || strcmp(expsetup.stim.esetup_exp_version{tid}, 'target & distractor all combinations')
    %==========
